...
1. ユーザーが、WebブラウザからWebサーバーに対してGETメソッドでURL(```/users```)を送信する。
1. Webサーバーに送信されたURLを基に、ルーティング（URLに基づいた経路の振り分け）を行い、```routes.rb```に定義された```users```コントローラーの```index```アクションを呼び出す。
...
### 2.1 新規作成ページを表示する
1. 一覧画面(```/tasks```)で新規登録ボタンを押すと、WebブラウザからWebサーバーに対してGETメソッドでURL（```/tasks/new```）が送信される。
1. 送信されたURLに基づき、ルーティング（URLに基づいた経路の振り分け）を行い、```routes.rb```に定義された```tasks```コントローラーの```new```アクションを呼び出す。
...
### 2.3 作成ボタンを押す
1. 「登録する」ボタンを押すと、今度はPOSTメソッドでURL（```/tasks```）がWebサーバーに送信される。このとき、```form_with```メソッドを使用したリクエストパラメータも送信する。
...
1. 送信されたURLを基づき、ルーティング（URLに基づいた経路を振り分け）を行い、```routes.rb```に定義された```tasks```コントローラーの```create```アクションを呼び出す。
